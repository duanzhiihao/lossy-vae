...
def qarv_cls_test(lmb_range=(16,2048), pretrained=False):
    ...
    cfg['dec_blocks'] = [
        # 1x1
        *[qarv.VRLatentBlockSmall(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        *[qarv.VRLatentBlockSmall(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2], kernel_size=3, mlp_ratio=3) for _ in range(2)],
        qarv_cls.ClassificationCutPoint(),
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[qarv.VRLatentBlockSmall(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3], kernel_size=5, mlp_ratio=2) for _ in range(3)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[qarv.VRLatentBlockSmall(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(3)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[4], _emb_dim, kernel_size=7, mlp_ratio=1.5) for _ in range(4)],
