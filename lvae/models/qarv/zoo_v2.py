...
@register_model
def qv2_4z_z128(lmb_range=(64,8192), pretrained=False):
    cfg = dict()
...
@register_model
def qv2_4z_z32(lmb_range=(64,8192), pretrained=False):
    cfg = dict()
...
@register_model
def qv2_4z_lowz(lmb_range=(64,8192), pretrained=False):
    cfg = dict()
...
@register_model
def qv2_4z_attn(lmb_range=(64,8192), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[res_block(dec_dims[0], kernel_size=3) for _ in range(2)],
        ...
        # 2x2
        *[res_block(dec_dims[1], kernel_size=5) for _ in range(2)],
        qarv.CrossAttnTransformerNCHW(dec_dims[1], kv_name='dec_s64', kv_dim=dec_dims[0]),
        qarv.VRLVBlockV2(dec_dims[1], z_dims[1], enc_key='enc_s32', enc_width=enc_dims[-2], kernel_size=5,
                         name='dec_s32'),
        *[res_block(dec_dims[1], kernel_size=5) for _ in range(2)],
        cm.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[res_block(dec_dims[2]) for _ in range(3)],
        qarv.CrossAttnTransformerNCHW(dec_dims[2], kv_name='dec_s64', kv_dim=dec_dims[0]),
        qarv.VRLVBlockV2(dec_dims[2], z_dims[2], enc_key='enc_s16', enc_width=enc_dims[-3],
                         name='dec_s16'),
        *[res_block(dec_dims[2]) for _ in range(3)],
        cm.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[res_block(dec_dims[3]) for _ in range(4)],
        qarv.CrossAttnTransformerNCHW(dec_dims[3], kv_name='dec_s64', kv_dim=dec_dims[0]),
        qarv.VRLVBlockV2(dec_dims[3], z_dims[3], enc_key='enc_s8', enc_width=enc_dims[-4],
                         name='dec_s8'),
        cm.CompresionStopFlag(), # no need to execute remaining blocks when compressing
        *[res_block(dec_dims[3]) for _ in range(4)],
