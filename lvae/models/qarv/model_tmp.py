...
class ConvNeXtBlockAdaLN(nn.Module):
    ...
    @torch.no_grad()
    def set_default_embedding(self, emb):
        embedding = self.embedding_layer(emb)
        shift, scale = torch.chunk(embedding, chunks=2, dim=-1)
        self.register_buffer('_default_shift', shift)
        self.register_buffer('_default_scale', scale)
    ...
    def forward(self, x, emb):
        ...
        # AdaLN
        if emb is None:
            shift, scale = self._default_shift, self._default_scale
        else:
            embedding = self.embedding_layer(emb)
            shift, scale = torch.chunk(embedding, chunks=2, dim=-1)
        x = x * (1 + scale) + shift
...
class VRLatentBlockBase(nn.Module):
    ...
    def set_default_embedding(self, emb):
        for m in [self.resnet_front, self.resnet_end,
                  self.posterior0, self.posterior1, self.posterior2]:
            m.set_default_embedding(emb)
...
class VRLatentBlockSmall(VRLatentBlockBase):
    ...
    def set_default_embedding(self, emb):
        for m in [self.resnet_front, self.resnet_end, self.posterior2]:
            m.set_default_embedding(emb)
...
class FeatureExtractor(nn.Module):
    ...
    def set_default_embedding(self, emb):
        for block in self.enc_blocks:
            if getattr(block, 'requires_embedding', False) or hasattr(block, 'set_default_embedding'):
                block.set_default_embedding(emb)
...
class VariableRateLossyVAE(nn.Module):
    ...
    def __init__(self, config: dict):
        super().__init__()
        # dummy variable used to infer the model device type
        ...
        self.dec_bias = nn.Parameter(torch.zeros(1, self.dec_blocks[0].in_channels, 1, 1))
        # lambda embedding module
        self._initialize_lmb_embedding(config)
        ...
        self._logging_images = config.get('log_images', []) # for training visualization

        # flags
        self._compressing = False
        self._flops_mode = False

    def _initialize_lmb_embedding(self, config):
        _low, _high = config['lmb_range']
        self.lmb_range = (float(_low), float(_high))
        assert self.lmb_range[0] > 1, f'lmb_range should be greater than 1, got {self.lmb_range}'
        ...
        self.set_default_embedding(self.lmb_range[1])

    def preprocess_input(self, im: torch.Tensor):
        """ Linear shift and scale the input image
    ...
    def preprocess_target(self, im: torch.Tensor):
        """ Shift and scale the ground truth, from (0,1) to (-1,1). Used during training.
    ...
    @torch.no_grad()
    def set_default_embedding(self, lmb, n=1):
        """ Set the default lmb embedding for all latent blocks so that \
            the model don't have to compute the embedding every time.

        Args:
            lmb (torch.Tensor, float, int): a scalar or a tensor of shape (n,)
        """
        self._default_lmb = lmb
        # self._default_lmb_emb = self._get_lmb_embedding(lmb, n=1)
        emb = self._get_lmb_embedding(lmb, n=n)
        self.encoder.set_default_embedding(emb)
    ...
    def expand_to_tensor(self, input_, n: int):
        """ expand a scalar to a tensor with shape (n,). \
        If input_ is already a tensor,
        - if it has only one element, then it will be expanded to (n,)
        - if it has n elements, then it will be returned as is
        - otherwise, an error will be raised

        Args:
            input_ (torch.Tensor, float, int): input variable
            n (int): number of elements in the output tensor
        """
    ...
    #     lmb_input = torch.log(lmb) / math.log(self.MAX_LMB) * self._sin_period
    #     return lmb_input

    def _get_lmb_embedding(self, lmb, n):
        lmb = self.expand_to_tensor(lmb, n=n)
        # scale lmb to the range (0, self._sin_period)
        scaled = torch.log(lmb) / math.log(self.MAX_LMB) * self._sin_period
    ...
    def get_bias(self, bhw_repeat=(1,1,1)):
        nB, nH, nW = bhw_repeat
        feature = self.dec_bias.expand(nB, -1, nH, nW)
        return feature
    ...
    def conditional_sample(self, lmb, latents, bhw_repeat=None, t=1.0):
        ...
        emb = None if (lmb is None) else self._get_lmb_embedding(lmb, n=nB)
        feature = self.get_bias(bhw_repeat=(nB, nH, nW))
        idx = 0
        for i, block in enumerate(self.dec_blocks):
            if getattr(block, 'is_latent_block', False):
    ...
    def study(self, save_dir, **kwargs):
        ...
        lmb = None # use default lmb embeddings
        ...
        if mode:
            for block in self.dec_blocks:
                if hasattr(block, 'update'):
                    block.update()
        self._compressing = mode

    @torch.no_grad()
    def compress(self, im, lmb=None):
        if (lmb is None) or (lmb == self._default_lmb):
            lmb = self._default_lmb
            emb = None # default lmb embeddings have been pre-computed
        else: # use new lambda
            emb = self._get_lmb_embedding(lmb, n=im.shape[0])
        ...
        for i, block in enumerate(self.dec_blocks):
            if getattr(block, 'is_latent_block', False):
                ...
                feature, stats = block(feature, emb, enc_feature=f_enc, mode='compress')
    ...
    def decompress(self, compressed_object):
        ...
        emb = None if (lmb == self._default_lmb) else self._get_lmb_embedding(lmb, n=nB)
        # lmb = self.expand_to_tensor(lmb, n=nB)
        ...
        for bi, block in enumerate(self.dec_blocks):
            if getattr(block, 'is_latent_block', False):
                ...
                feature, _ = block(feature, emb, mode='decompress', strings=strs_batch)
                str_i += 1
            elif getattr(block, 'requires_embedding', False):
                feature = block(feature, emb)
            else:
