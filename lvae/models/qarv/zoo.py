...
@register_model
def qarv_b3_mlp_k(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        # 1x1
        *[qarv.VRLVBlockR2P3(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=2) for _ in range(1)],
        qarv.ConvNeXtBlockAdaLN(dec_dims[0], _emb_dim, kernel_size=1, mlp_ratio=2),
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        qarv.ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim, kernel_size=3, mlp_ratio=3),
        *[qarv.VRLVBlockR2P3(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2], kernel_size=3, mlp_ratio=3) for _ in range(2)],
        qarv.ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim, kernel_size=3, mlp_ratio=3),
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        qarv.ConvNeXtBlockAdaLN(dec_dims[2], _emb_dim, kernel_size=5, mlp_ratio=2),
        *[qarv.VRLVBlockR2P3(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3], kernel_size=5, mlp_ratio=2) for _ in range(3)],
        qarv.ConvNeXtBlockAdaLN(dec_dims[2], _emb_dim, kernel_size=5, mlp_ratio=2),
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        qarv.ConvNeXtBlockAdaLN(dec_dims[3], _emb_dim, kernel_size=7, mlp_ratio=1.75),
        *[qarv.VRLVBlockR2P3(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(3)],
        qarv.ConvNeXtBlockAdaLN(dec_dims[3], _emb_dim, kernel_size=7, mlp_ratio=1.75),
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[4], _emb_dim, kernel_size=7, mlp_ratio=1.5) for _ in range(8)],
...
@register_model
def qarv_b2(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        # 1x1
        *[qarv.LatentBlockBase(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        qarv.ConvNeXtBlockAdaLN(dec_dims[0], _emb_dim, kernel_size=1, mlp_ratio=4),
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        qarv.ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim, kernel_size=3, mlp_ratio=3),
...
@register_model
def qarv_9b(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    ch = 128
    enc_dims = [192, ch*3, ch*4, ch*4, ch*2]
    dec_dims = [ch*2, ch*4, ch*3, ch*2, ch*1]
    z_dims = [32, 32, 96, 8]
...
@register_model
def qarv_s4(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[qarv.LatentBlockSmall(dec_dims[0], z_dims[0], enc_width=enc_dims[-1]) for _ in range(1)],
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        *[qarv.VRLatentBlockSmall(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2]) for _ in range(1)],
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[qarv.VRLatentBlockSmall(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(2)],
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[2], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[qarv.VRLatentBlockSmall(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(2)],
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[3], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[qarv.ConvNeXtBlockAdaLN(dec_dims[4], _emb_dim) for _ in range(4)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
...
@register_model
def qarv_s5(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['enc_blocks'] = [
        ...
        common.patch_downsample(im_channels, enc_dims[0], rate=4),
        # 16x16
        *[res_block(enc_dims[0], _emb_dim) for _ in range(4)],
        qarv.ConvNeXtAdaLNPatchDown(enc_dims[0], enc_dims[1], embed_dim=_emb_dim),
        # 8x8
        *[res_block(enc_dims[1], _emb_dim) for _ in range(4)],
        qarv.ConvNeXtAdaLNPatchDown(enc_dims[1], enc_dims[2], embed_dim=_emb_dim),
        # 4x4
        *[res_block(enc_dims[2], _emb_dim) for _ in range(4)],
        qarv.ConvNeXtAdaLNPatchDown(enc_dims[2], enc_dims[3], embed_dim=_emb_dim),
        # 2x2
        *[res_block(enc_dims[3], _emb_dim) for _ in range(4)],
        qarv.ConvNeXtAdaLNPatchDown(enc_dims[3], enc_dims[4], embed_dim=_emb_dim),
        # 1x1
        *[res_block(enc_dims[4], _emb_dim) for _ in range(4)],
    ]
    ...
    cfg['dec_blocks'] = [
        ...
        *[qarv.LatentBlockSmall(dec_dims[0], z_dims[0], enc_width=enc_dims[-1]) for _ in range(1)],
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        *[res_block(dec_dims[1], _emb_dim) for _ in range(1)],
        *[qarv.VRLVBlockR1P1(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2]) for _ in range(1)],
        *[res_block(dec_dims[1], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[res_block(dec_dims[2], _emb_dim) for _ in range(2)],
        *[qarv.VRLVBlockR1P1(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(2)],
        *[res_block(dec_dims[2], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[res_block(dec_dims[3], _emb_dim) for _ in range(2)],
        *[qarv.VRLVBlockR1P1(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(2)],
        *[res_block(dec_dims[3], _emb_dim) for _ in range(2)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[res_block(dec_dims[4], _emb_dim) for _ in range(4)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
    ]
