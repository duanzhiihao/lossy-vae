...
class VariableRateLossyVAE(nn.Module):
    ...
    def _setup_lmb_embedding(self, config):
        self.register_buffer('lmb_values', config['lmb_values'])
        self.lmb_values: torch.Tensor
        self.default_lmb_idx = -1
        self.lmb_embed_dim = config['lmb_embed_dim']
        self.lmb_embedding = nn.Embedding(self.lmb_values.shape[0], self.lmb_embed_dim)
    ...
    def sample_lmb(self, n):
        lmb = torch.randint(0, self.lmb_values.shape[0], size=(n,), device=self._dummy.device)
        assert lmb.shape == (n,) and lmb.dtype == torch.int64
        return lmb
    ...
    def expand_to_tensor(self, input_, n):
        ...
        if isinstance(input_, (float, int)):
            input_ = torch.full(size=(n,), fill_value=int(input_),
                                dtype=torch.int64, device=self._dummy.device)
        assert input_.shape == (n,), f'{input_=}, {input_.shape=}'
        return input_

    def _get_lmb_embedding(self, lmb_idx, n):
        lmb_idx = self.expand_to_tensor(lmb_idx, n=n)
        embedding = self.lmb_embedding(lmb_idx)
        return embedding
    ...
    def forward(self, batch, lmb=None, return_rec=False):
        ...
        # rate + distortion
        loss = kl + self.lmb_values[lmb] * distortion
        loss = loss.mean(0)
    ...
    #     lmb = self.expand_to_tensor(self.default_lmb_idx, n=1)
    ...
    @torch.no_grad()
    def _self_evaluate(self, img_paths, lmb: int, pbar=False, log_dir=None):
        ...
        for impath in pbar:
            ...
            x_hat, stats_all = self.forward_end2end(im, lmb=self.expand_to_tensor(int(lmb),n=1))
            # compute bpp
        ...
        avg_stats['lambda'] = int(self.lmb_values[lmb])
        if log_dir is not None:
    ...
    def self_evaluate(self, img_dir, lmb_range=None, steps=8, log_dir=None):
        ...
        pbar = tqdm(self.lmb_values.tolist(), position=0, ascii=True)
        all_lmb_stats = defaultdict(list)
        if log_dir is not None:
            (Path(log_dir) / 'all_lmb_channel_stats.txt').unlink(missing_ok=True)
        for lmbi, lmb in enumerate(pbar):
            assert isinstance(lmb, float)
            results = self._self_evaluate(img_paths, lmbi, log_dir=log_dir)
...
@register_model
def qarvb_cconv(pretrained=False):
    cfg = dict()

    # variable rate
    cfg['lmb_values'] = torch.Tensor([16, 32, 64, 128, 256, 512, 1024, 2048])
    cfg['lmb_embed_dim'] = 256
    # cfg['sin_period'] = 64
    _emb_dim = cfg['lmb_embed_dim']
    ...
    cfg['enc_blocks'] = [
        ...
        # 16x16
        *[ConvNeXtBlockAdaLN(enc_dims[0], _emb_dim, kernel_size=7) for _ in range(6)],
        ConvNeXtAdaLNPatchDown(enc_dims[0], enc_dims[1], embed_dim=_emb_dim),
        # 8x8
        *[ConvNeXtBlockAdaLN(enc_dims[1], _emb_dim, kernel_size=7) for _ in range(6)],
        ConvNeXtAdaLNPatchDown(enc_dims[1], enc_dims[2], embed_dim=_emb_dim),
        # 4x4
        *[ConvNeXtBlockAdaLN(enc_dims[2], _emb_dim, kernel_size=5) for _ in range(6)],
        ConvNeXtAdaLNPatchDown(enc_dims[2], enc_dims[3], embed_dim=_emb_dim),
        # 2x2
        *[ConvNeXtBlockAdaLN(enc_dims[3], _emb_dim, kernel_size=3) for _ in range(4)],
        ConvNeXtAdaLNPatchDown(enc_dims[3], enc_dims[3], embed_dim=_emb_dim),
        # 1x1
        *[ConvNeXtBlockAdaLN(enc_dims[3], _emb_dim, kernel_size=1) for _ in range(4)],
    ]

    cfg['dec_blocks'] = [
        # 1x1
        *[VRLatentBlockBase(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        ConvNeXtBlockAdaLN(dec_dims[0], _emb_dim, kernel_size=1, mlp_ratio=4),
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim, kernel_size=3, mlp_ratio=3),
        *[VRLatentBlockBase(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2], kernel_size=3, mlp_ratio=3) for _ in range(2)],
        ConvNeXtBlockAdaLN(dec_dims[1], _emb_dim, kernel_size=3, mlp_ratio=3),
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        ConvNeXtBlockAdaLN(dec_dims[2], _emb_dim, kernel_size=5, mlp_ratio=2),
        *[VRLatentBlockBase(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3], kernel_size=5, mlp_ratio=2) for _ in range(3)],
        ConvNeXtBlockAdaLN(dec_dims[2], _emb_dim, kernel_size=5, mlp_ratio=2),
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        ConvNeXtBlockAdaLN(dec_dims[3], _emb_dim, kernel_size=7, mlp_ratio=1.75),
        *[VRLatentBlockBase(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(3)],
        ConvNeXtBlockAdaLN(dec_dims[3], _emb_dim, kernel_size=7, mlp_ratio=1.75),
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[ConvNeXtBlockAdaLN(dec_dims[4], _emb_dim, kernel_size=7, mlp_ratio=1.5) for _ in range(8)],
