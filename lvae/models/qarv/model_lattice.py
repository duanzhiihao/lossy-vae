...
class VRLVBlockBase(nn.Module):
    ...
    def forward(self, feature, lmb_embedding, prior_model, enc_feature=None, mode='trainval',
...
class VariableRateLossyVAE(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.prior_model = config.pop('prior_model')
    ...
    def forward_end2end(self, im: torch.Tensor, lmb: torch.Tensor, mode='trainval', get_latent=False):
        ...
        for i, block in enumerate(self.dec_blocks):
            if getattr(block, 'is_latent_block', False):
                ...
                feature, stats = block(feature, emb, self.prior_model,
                                       enc_feature=f_enc, mode=mode, get_latent=get_latent)
...
@register_model
def qarvb_a2(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        # 1x1
        *[VRLVBlockBase(dec_dims[0], z_dims[0], enc_key='enc_s64', enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        ...
        *[VRLVBlockBase(dec_dims[1], z_dims[1], enc_key='enc_s32', enc_width=enc_dims[-2], kernel_size=3, mlp_ratio=3) for _ in range(2)],
        res_block(dec_dims[1], kernel_size=3, mlp_ratio=3),
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        res_block(dec_dims[2], kernel_size=5, mlp_ratio=2),
        *[VRLVBlockBase(dec_dims[2], z_dims[2], enc_key='enc_s16', enc_width=enc_dims[-3], kernel_size=5, mlp_ratio=2) for _ in range(3)],
        res_block(dec_dims[2], kernel_size=5, mlp_ratio=2),
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        res_block(dec_dims[3], kernel_size=7, mlp_ratio=1.75),
        *[VRLVBlockBase(dec_dims[3], z_dims[3], enc_key='enc_s8', enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(3)],
    ...
    from laic.models.prior_models import pma2_siren_af_h64
    net = pma2_siren_af_h64(xdim=2)
    load_weights(net, f"{url_root}/lattice/pma2_siren_af_h64_2-1m.pt")
    cfg["prior_model"] = NeuralCovPriorV2(net, lattice="a2")
...
def load_weights(model: nn.Module, url: str):
    if url.startswith('http'):
        checkpoint = torch.hub.load_state_dict_from_url(url, weights_only=True)
    else:
        checkpoint = torch.load(url, weights_only=True)
