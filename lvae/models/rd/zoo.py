...
@register_model
def rd_model_c192(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    enc_dims = [192, 384, 512, 512, 512]
    dec_dims = [512, 512, 512, 384, 192]
    z_dims = [32, 32, 32, 32, 32]
...
@register_model
def rd_model_e(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    ch = 128
    enc_dims = [96, 192, ch*3, ch*4, ch*4, ch*4]
    dec_dims = [ch*4, ch*4, ch*3, ch*2, ch*1, ch//2]
    z_dims = [32, 32, 32, 32, 32, 16]

    im_channels = 3
    cfg['enc_blocks'] = [
        # 64x64
        common.patch_downsample(im_channels, enc_dims[0], rate=2),
        # 32x32
        *[lib.ConvNeXtBlockAdaLN(enc_dims[0], _emb_dim) for _ in range(4)],
        lib.ConvNeXtAdaLNPatchDown(enc_dims[0], enc_dims[1], embed_dim=_emb_dim),
        # 16x16
        *[lib.ConvNeXtBlockAdaLN(enc_dims[1], _emb_dim) for _ in range(6)],
        lib.ConvNeXtAdaLNPatchDown(enc_dims[1], enc_dims[2], embed_dim=_emb_dim),
        ...
        *[lib.ConvNeXtBlockAdaLN(enc_dims[2], _emb_dim) for _ in range(6)],
        ...
        # 4x4
        *[lib.ConvNeXtBlockAdaLN(enc_dims[3], _emb_dim) for _ in range(6)],
        lib.ConvNeXtAdaLNPatchDown(enc_dims[3], enc_dims[4], embed_dim=_emb_dim),
        # 2x2
        *[lib.ConvNeXtBlockAdaLN(enc_dims[4], _emb_dim) for _ in range(4)],
        lib.ConvNeXtAdaLNPatchDown(enc_dims[4], enc_dims[5], embed_dim=_emb_dim),
        # 1x1
        *[lib.ConvNeXtBlockAdaLN(enc_dims[5], _emb_dim) for _ in range(4)],
    ]
