...
def kl_with_logstd(mu1, logv1, mu2, logv2):
    ...
    Args:
        mu1   (torch.tensor): mean 1
        logv1 (torch.tensor): log-std 1
        mu2   (torch.tensor): mean 2
        logv2 (torch.tensor): log-std 2
    """
    return -0.5 + logv2 - logv1 + 0.5 * (logv1.exp() ** 2 + (mu1 - mu2) ** 2) / (logv2.exp() ** 2)
...
class VRLatentBlock3Pos(nn.Module):
    ...
    def transform_posterior(self, feature, enc_feature, lmb_embedding):
        ...
        qm, qlogv = self.posterior(merged).chunk(2, dim=1)
        return qm, qlogv
    ...
    def forward(self, feature, lmb_embedding, enc_feature=None, mode='trainval',
        ...
        Args:
            ...
            enc_feature (torch.Tensor): feature map
        """
        feature, pm, plogv = self.transform_prior(feature, lmb_embedding)
