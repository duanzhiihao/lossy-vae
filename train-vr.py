...
import wandb
from timm.utils import unwrap_model, random_seed

import utils
import utils.ddp as ddputils
import utils.lr_schedulers as lr_schedulers
from utils.torch_utils import load_partial
from datasets.compression import get_bd_rate_over_anchor
from datasets.image import get_dateset, datasets_root
...
def parse_args():
    ...
    # model setting
    parser.add_argument('--model',      type=str,  default='tpc_tiny')
...
class BaseTrainWrapper():
    ...
    def set_device(self):
        ...
        if world_size == 1: # standard single GPU mode
            ...
            logging.add_file_handler(fpath=self._log_dir / f'logs.txt')
        else: # DDP mode
            ...
            logging.add_file_handler(fpath=self._log_dir / f'logs_rank{local_rank}.txt')

            with ddputils.run_sequentially():
                msg = f'local_rank={local_rank}, world_size={world_size}, total visible={_count}'
                logging.log(logging.FORCE, msg)
                logging.log(logging.FORCE, f'{torch.cuda.get_device_properties(local_rank)} \n')
            torch.distributed.barrier()
    ...
    def prepare_configs(self):
        ...
        logging.log(logging.HIGHLIGHT, msg)
    ...
    def set_pretrain(self):
        ...
        if cfg.resume is not None: # resume
            ...
            load_partial(self.model, checkpoint)
            if cfg.load_optim:
                self.optimizer.load_state_dict(checkpoint['optimizer'])
                self.scaler.load_state_dict(checkpoint['scaler'])
                for param_group in self.optimizer.param_groups:
    ...
    def gradient_clip_(self, parameters):
        ...
        if grad_norm > (moving_median * 10): # super large gradient
            ...
            # logging.warning(f'Large gradient norm = {grad_norm:3f}. Clipping to {_clip:.3f} ...')
            for param_group in self.optimizer.param_groups:
                param_group['lr'] *= 0.1
            _lr = param_group['lr']
    ...
    def evaluate(self):
        ...
        logging.log_dict_as_table(results)
...
class TrainWrapper(BaseTrainWrapper):
